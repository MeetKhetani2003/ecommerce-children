...
export default function CreateOfflineOrderPage() {
  ...
  const handleSubmitOrder = async (e: React.FormEvent) => {
    ...
    if (cartItems.length === 0) {
      setError("Please add at least one costume item to the order.");
      return;
    }
  ...
  if (!session) {
    return (
      <div className="mx-auto max-w-[500px] px-4 py-20 text-center">
        ...
        <p className="mt-2 text-[14px] text-[#6B5A6F]">Please sign in to access the administrator console.</p>
  ...
  if (!isAdmin) {
    return (
      <div className="mx-auto max-w-[500px] px-4 py-20 text-center">
        ...
        <p className="mt-2 text-[14px] text-[#6B5A6F]">Your account does not have admin permissions.</p>
  ...
  return (
    <div className="mx-auto max-w-[1000px] px-4 py-8 md:py-12">
      <div className="mb-6">
        <Link href="/admin?tab=orders" className="inline-flex items-center gap-2 text-[14px] font-medium text-[#8B1D8F] hover:underline">
          <ArrowLeft className="h-4 w-4" /> Back to Orders
        </Link>
      </div>

      <div className="mb-8">
        <h1 className="text-[28px] font-semibold tracking-tight text-[#1A0F1C]">Create Offline Order</h1>
        <p className="text-[14px] text-[#6B5A6F]">Place a custom order for walk-in/counter sales. Inventory stocks will deduct automatically.</p>
      </div>
      ...
      <form onSubmit={handleSubmitOrder} className="grid gap-8 md:grid-cols-[1.2fr_1fr]">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-3xl border border-[#F0E6F2] bg-white p-6 shadow-sm">
            <h2 className="text-[17px] font-semibold text-[#1A0F1C] mb-4">Customer Details</h2>
            <div className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <label className="mb-1 block text-[12.5px] font-medium text-[#4A354D]">Customer Name *</label>
                  <input
                    type="text"
                    required
                    value={customerName}
                ...
                <div>
                  <label className="mb-1 block text-[12.5px] font-medium text-[#4A354D]">Phone Number *</label>
                  <input
                    type="tel"
                    required
                    value={customerPhone}
              ...
              <div>
                <label className="mb-1 block text-[12.5px] font-medium text-[#4A354D]">Email Address (Optional)</label>
                <input
                  type="email"
                  value={customerEmail}
              ...
              <div>
                <label className="mb-1 block text-[12.5px] font-medium text-[#4A354D]">Shipping/Store Address</label>
                <textarea
                  rows={2}
                  value={customerAddress}
                  onChange={(e) => setCustomerAddress(e.target.value)}
                  placeholder="Street / Walk-in Purchase Counter"
                  className="w-full rounded-xl border border-[#EEDDF0] p-3 text-[13.5px] focus:outline-[#8B1D8F] bg-white text-[#1A0F1C]"
                />
              </div>
            </div>
          </div>
            ...
            {loadingProducts ? (
              ...
              <div className="space-y-4">
                <div className="relative">
                  <label className="mb-1 block text-[12.5px] font-medium text-[#4A354D]">Select Costume (Searchable)</label>
                  <div className="flex gap-2">
                    <input
                      type="text"
                      placeholder="Type costume title or category to search..."
                      value={searchQuery}
                      onChange={(e) => {
                      ...
                      className="h-11 flex-1 rounded-xl border border-[#EEDDF0] px-3.5 text-[13.5px] focus:outline-[#8B1D8F] bg-white text-[#1A0F1C]"
                    />
                    {selectedProductId !== "" && (
                      <button
                        type="button"
                  ...
                  {showDropdown && (
                    <div className="absolute z-20 mt-1 max-h-[200px] w-full overflow-y-auto rounded-xl border border-[#F0E6F2] bg-white shadow-lg">
                      {filteredProducts.length === 0 ? (
                        <div className="p-3 text-[13.5px] text-gray-400 italic">No costumes found</div>
                      ) : (
                        filteredProducts.map(p => (
                          ...
                          >
                            <div>
                              <span className="font-semibold text-[#1A0F1C]">{p.title}</span>
                              <span className="ml-2 inline-block rounded bg-[#FCF7FD] px-1.5 py-0.5 text-[10px] text-[#8B7A8F]">{p.category}</span>
                            </div>
                            <div className="text-right shrink-0">
                              <span className="font-bold text-[#8B1D8F]">₹{p.price}</span>
                              <span className="ml-2 text-[11px] text-[#8B7A8F]">Stock: {p.stock}</span>
                            </div>
                          </button>
                        ))
                      )}
                ...
                {selectedProductId !== "" && (
                  <div className="flex items-end gap-4">
                    ...
                    <button
                      ...
                      className="h-10.5 rounded-xl bg-[#8B1D8F] px-5 text-[13.5px] font-semibold text-white hover:bg-[#7A187C] transition flex items-center gap-1.5"
                    >
        ...
        <div className="space-y-6">
          <div className="rounded-3xl border border-[#F0E6F2] bg-[#FCF7FD]/70 p-6 shadow-sm">
            <h2 className="text-[18px] font-semibold text-[#1A0F1C] mb-4">Cart List</h2>
                ...
                {cartItems.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-3 rounded-2xl border border-white bg-white/60 p-2.5">
                    ...
                    <button
                      ...
                      className="p-1.5 text-[#A38AA6] hover:text-red-500 rounded-lg hover:bg-red-50"
                    >
            ...
            {/* Calculations & Fields */}
            <div className="border-t border-[#E9D5ED] pt-4 space-y-4 text-[13.5px] text-[#4A354D]">
              ...
              <div>
                <label className="mb-1 block text-[12px] font-medium text-[#8B7A8F]">Manual Discount Amount (₹)</label>
                <input
                  type="number"
                  min={0}
                  max={subtotal}
                  ...
                  className="h-9 w-full rounded-lg border border-[#EEDDF0] px-3 text-[13px] bg-white text-[#1A0F1C]"
                />
              </div>

              <div className="flex justify-between font-bold text-[#1A0F1C] text-[16px] border-t border-[#E9D5ED] pt-3">
            ...
            <div className="mt-6 space-y-4 border-t border-[#E9D5ED] pt-4">
              <div className="grid gap-3 grid-cols-2">
                <div>
                  <label className="mb-1 block text-[12px] font-medium text-[#8B7A8F]">Payment Status</label>
                  <select
                    value={paymentStatus}
                    onChange={(e) => setPaymentStatus(e.target.value)}
                    className="h-9.5 w-full rounded-xl border border-[#EEDDF0] bg-white px-2.5 text-[12.5px] text-[#1A0F1C] cursor-pointer"
                  >
                ...
                <div>
                  <label className="mb-1 block text-[12px] font-medium text-[#8B7A8F]">Shipping Status</label>
                  <select
            ...
            <button
              ...
              className="mt-6 w-full rounded-full bg-[#8B1D8F] py-3 text-[14.5px] font-bold text-white transition hover:bg-[#7A187C] disabled:opacity-50"
            >
