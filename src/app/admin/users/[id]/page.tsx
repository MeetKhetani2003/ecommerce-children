...
export default function UserDetailPage() {
  ...
  if (!session) {
    return (
      <div className="mx-auto max-w-[500px] px-4 py-20 text-center">
        ...
        <p className="mt-2 text-[14px] text-[#6B5A6F]">Please sign in to access the admin panel.</p>
  ...
  if (!isAdmin) {
    return (
      <div className="mx-auto max-w-[500px] px-4 py-20 text-center">
        ...
        <p className="mt-2 text-[14px] text-[#6B5A6F]">You do not have permission to view user accounts.</p>
  ...
  return (
    <div className="mx-auto max-w-[1240px] px-4 py-8 md:py-12">
      <div className="mb-6">
        <Link href="/admin?tab=users" className="inline-flex items-center gap-2 text-[14px] font-medium text-[#8B1D8F] hover:underline">
          <ArrowLeft className="h-4 w-4" /> Back to Dashboard (Users)
        </Link>
      </div>

      {loading ? (
        <div className="flex h-[400px] items-center justify-center text-[14px] text-[#8B7A8F]">
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_2fr]">
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-3xl border border-[#F0E6F2] bg-white p-6 shadow-sm">
              <div className="flex flex-col items-center text-center">
                {userData.image ? (
                  <img src={userData.image} alt={userData.name} className="h-24 w-24 rounded-full border-2 border-[#8B1D8F] object-cover mb-4" />
                ) : (
                  <div className="grid h-20 w-20 place-items-center rounded-full bg-gradient-to-br from-[#8B1D8F] to-[#E91E7A] text-[28px] font-bold text-white mb-4">
                    {userData.name.split(" ").map((n: string) => n[0]).join("")}
                  </div>
                )}
                <h2 className="text-[20px] font-bold text-[#1A0F1C]">{userData.name}</h2>
                ...
                <div className="mt-6 w-full space-y-4 border-t border-[#F8F0F9] pt-4 text-left text-[13.5px] text-[#4A354D]">
                  <div className="flex items-center gap-3">
                    <Mail className="h-4 w-4 text-[#8B7A8F] shrink-0" />
                  ...
                  <div className="flex items-center gap-3">
                    <Shield className="h-4 w-4 text-[#8B7A8F] shrink-0" />
            ...
            {/* Address Book Card */}
            <div className="rounded-3xl border border-[#F0E6F2] bg-white p-6 shadow-sm">
              <h3 className="mb-4 text-[16px] font-semibold text-[#1A0F1C] flex items-center gap-2">
                <MapPin className="h-4.5 w-4.5 text-[#8B1D8F]" /> Saved Address Book
              </h3>
              {userData.addresses && userData.addresses.length > 0 ? (
                <ul className="space-y-3">
                  {userData.addresses.map((addr: string, i: number) => (
                    <li key={i} className="text-[13px] text-[#4A354D] flex items-start gap-2.5 rounded-2xl border border-[#F0E6F2] p-3 bg-[#FFFCFE]">
                      <div className={`mt-1 h-2 w-2 rounded-full shrink-0 ${addr === userData.defaultAddress ? "bg-[#8B1D8F]" : "bg-[#D6C4D8]"}`} />
                      <div className="flex-1 leading-relaxed">
                        {addr}
                        {addr === userData.defaultAddress && (
                          <span className="ml-2 inline-block rounded bg-[#F3E7F5] px-1.5 py-0.5 text-[9.5px] font-bold text-[#8B1D8F] border border-[#E9D5ED]">
                            Default
                          </span>
                        )}
                      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="rounded-3xl border border-[#F0E6F2] bg-white p-6 shadow-sm">
                <h3 className="mb-4 text-[16px] font-semibold text-[#1A0F1C] flex items-center gap-2">
                  <ShoppingCart className="h-4.5 w-4.5 text-[#8B1D8F]" /> Synced Cart
                </h3>
                {userData.cart && userData.cart.length > 0 ? (
                  <div className="space-y-3 max-h-[250px] overflow-y-auto pr-1">
                    {userData.cart.map((item: any, i: number) => (
                      <div key={i} className="flex items-center gap-3 rounded-xl border border-[#F8F0F9] p-2 hover:bg-[#FCF7FD]/30 transition">
                        ...
                        <span className="text-[13px] font-bold text-[#1A0F1C]">₹{item.price * item.quantity}</span>
                      </div>
                    ))}
                  </div>
                ) : (
                ...
                {userData.wishlist && userData.wishlist.length > 0 ? (
                  <div className="space-y-3 max-h-[250px] overflow-y-auto pr-1">
                    {userData.wishlist.map((wId: number, i: number) => {
                      ...
                      return (
                        <div key={i} className="flex items-center gap-3 rounded-xl border border-[#F8F0F9] p-2 hover:bg-red-50/10 transition">
                          {prod ? (
                            <>
                              <img src={prod.image} alt="" className="h-10 w-9 rounded object-cover bg-gray-50 border shrink-0" />
                              <div className="flex-1 min-w-0">
                                <h4 className="text-[13px] font-semibold text-[#1A0F1C] truncate">{prod.title}</h4>
                                <p className="text-[11px] text-[#8B7A8F]">Category: {prod.category}</p>
                              </div>
                              <span className="text-[13.5px] font-semibold text-[#8B1D8F]">₹{prod.price}</span>
                            </>
                          ) : (
                            <div className="text-[12px] text-gray-500">Costume ID: {wId}</div>
                          )}
                        </div>
                      );
                    })}
            ...
            {/* Orders History Card */}
            <div className="rounded-3xl border border-[#F0E6F2] bg-white p-6 shadow-sm">
              <h3 className="mb-5 text-[17px] font-semibold text-[#1A0F1C] flex items-center gap-2">
                <Package className="h-5 w-5 text-[#8B1D8F]" /> Order History ({orders.length})
              </h3>
              {orders.length === 0 ? (
                <p className="text-[14px] text-center text-[#8B7A8F] py-8">No orders placed by this user yet.</p>
              ) : (
                <div className="space-y-5">
                  {orders.map((order) => (
                    <div key={order._id} className="rounded-2xl border border-[#F0E6F2] p-5 hover:border-[#E1BFE6] transition">
                      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#F8F0F9] pb-3 mb-4">
                        <div>
                          <div className="text-[13px] text-[#8B7A8F]">
                            Order ID: <span className="font-mono text-[#8B1D8F] font-semibold">{order._id}</span>
                          </div>
                      ...
                      <div className="grid gap-4 md:grid-cols-3">
                        ...
                        <div className="rounded-xl bg-[#FCF7FD] p-3 text-[12.5px] border border-[#F0E6F2]">
                          <div className="font-semibold text-[#1A0F1C] mb-1.5">Delivery Details</div>
                          ...
                          <div className="mt-2 border-t border-[#EEDDF0] pt-1.5 flex justify-between font-bold text-[#1A0F1C]">
                      ...
                      <div className="mt-4 border-t border-[#F8F0F9] pt-3 flex items-center justify-between gap-4">
                        <div className="flex items-center gap-2 text-[12.5px] text-[#6B5A6F] w-full max-w-[320px]">
                          <span>Tracking #:</span>
                          <input 
                            type="text" 
                            ...
                            className="flex-1 rounded-lg border border-[#EEDDF0] px-2.5 py-1 text-[12.5px] focus:outline-[#8B1D8F] bg-white text-[#1A0F1C]"
                          />
                        </div>
                        <div className="text-[11.5px] text-[#8B7A8F] italic">Press Enter / Click away to save tracking #</div>
                      </div>
                    </div>
                  ))}
                </div>
