  ...
  if (cartItems.length === 0) {
    return (
      <div className="mx-auto flex max-w-[1240px] flex-col items-center justify-center px-4 py-20 text-center md:py-32">
        ...
        <Link to="/" className="mt-8 rounded-full bg-[#8B1D8F] px-8 py-3.5 text-[15px] font-medium text-white transition hover:bg-[#7A187C]">
          Start Shopping
        </Link>
      </div>
    );
        ...
        <div className="space-y-4">
          {cartItems.map((item) => (
            <div key={item.id} className="flex gap-4 rounded-2xl border border-[#F0E6F2] p-4 transition-all hover:shadow-md">
              <Link to={`/product/${item.id}`} className="block h-[120px] w-[100px] shrink-0 overflow-hidden rounded-xl bg-[#FCF7FD]">
                <img src={item.image} alt={item.title} className="h-full w-full object-cover" />
              </Link>
              <div className="flex flex-1 flex-col justify-between py-1">
                <div className="flex justify-between gap-2">
                  <div>
                    <Link to={`/product/${item.id}`} className="text-[15px] font-medium text-[#1A0F1C] hover:text-[#8B1D8F]">{item.title}</Link>
