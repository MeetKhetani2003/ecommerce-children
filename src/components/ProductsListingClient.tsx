...
export default function ProductsListingClient({
  ...
  const filteredProducts = useMemo(() => {
    ...
    // 2. City filter (Local SEO)
    if (initialCity && initialCity.toLowerCase() !== "all") {
      result = result.filter(p => {
        const productCities = p.cities || ["All"];
        return (
          productCities.some((c: string) => c.toLowerCase() === "all") ||
          productCities.some((c: string) => c.toLowerCase() === initialCity.toLowerCase())
        );
      });
    }
    ...
    if (initialBrand) {
      result = result.filter(p => {
        const productBrand = p.brand || "Saheli Shrungar";
        return (
          slugify(productBrand) === initialBrand.toLowerCase() ||
          productBrand.toLowerCase() === initialBrand.toLowerCase()
        );
      });
    }
    ...
    if (activeAges.length > 0) {
      ...
      result = result.filter(p => {
        return activeAges.some(age => {
          // Check if sizes exist and contain the age group
          if (p.sizes && p.sizes.length > 0) {
            return p.sizes.some((s: any) => s.size.toLowerCase().includes(age.toLowerCase().split(" ")[0]));
          }
          return false;
        });
      });
    ...
    return result;
  }, [productsList, activeCategory, initialCity, initialBrand, sortBy, activeAges, activePrices]);
  ...
  const headingText = useMemo(() => {
    ...
    if (initialCity) {
      return `${text} in ${formatStr(initialCity)}`;
    }
    if (initialBrand) {
      const brandName = initialBrand === "saheli-shrungar" ? "Saheli Shrungar" : formatStr(initialBrand);
    ...
    return text;
  }, [activeCategory, initialCity, initialBrand]);
    ...
    <div className="mx-auto max-w-[1240px] px-4 py-8 md:py-12">
      <div className="mb-8 md:flex md:items-end md:justify-between">
        <div>
          ...
          {initialCity && (
            <p className="mt-1 text-[14px] font-semibold text-[#8B1D8F]">
              📍 Fast Next-Day Delivery & COD Available in {initialCity.charAt(0).toUpperCase() + initialCity.slice(1)}
            </p>
          )}
